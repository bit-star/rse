...
@Component({
  selector: 'jhi-item-lease-cycle-rse',
  templateUrl: './item-lease-cycle-rse.component.html'
})
export class ItemLeaseCycleRseComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.itemLeaseCycleService
      ...
      .subscribe(
        (res: HttpResponse<IItemLeaseCycleRse[]>) => this.paginateItemLeaseCycles(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/item-lease-cycle-rse'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IItemLeaseCycleRse) {
    return item.id;
  }
  ...
  protected paginateItemLeaseCycles(data: IItemLeaseCycleRse[], headers: HttpHeaders) {

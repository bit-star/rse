...
@Component({
  selector: 'jhi-alipay-freeze-response-rse',
  templateUrl: './alipay-freeze-response-rse.component.html'
})
export class AlipayFreezeResponseRseComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.alipayFreezeResponseService
      ...
      .subscribe(
        (res: HttpResponse<IAlipayFreezeResponseRse[]>) => this.paginateAlipayFreezeResponses(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/alipay-freeze-response-rse'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IAlipayFreezeResponseRse) {
    return item.id;
  }
  ...
  protected paginateAlipayFreezeResponses(data: IAlipayFreezeResponseRse[], headers: HttpHeaders) {
